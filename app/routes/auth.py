...
async def register_page(request: Request, db: Session = Depends(get_db)):
    """Display registration page with dynamic hospitals list"""
    hospitals = db.query(Customer).filter(Customer.is_active == True).order_by(Customer.name).all()
    return templates.TemplateResponse("register.html", {"request": request, "hospitals": hospitals})
...
async def register_form(
    ...
    # Load hospitals for re-rendering on validation errors
    from app.models.models import Customer
    hospitals = db.query(Customer).filter(Customer.is_active == True).order_by(Customer.name).all()
